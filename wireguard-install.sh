...
#
# Copyright (c) 2023 Artem Nazarov <art_nazarov@mail.ru>
...
Copyright (c) 2023 Artem Nazarov
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
	...
	apk add iptables curl wget iproute2
	# install_wget
	# install_iproute
	show_start_setup
	detect_ip
	...
	if [[ "$os" == "ubuntu" ]]; then
		...
		apk add libqrencode qrencode wireguard-tools ca-certificates
		mkdir -p /etc/wireguard/
	fi
