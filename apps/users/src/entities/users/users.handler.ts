...
class UsersCommandsHandler implements ICommandHandler {
	...
	async handle(context: UsersContext, message: Message, from: User, start = false) {
		...
		if (context[CmdCode.Command] === VPNUserCommand.ShowInstallationGuide) {
			await this.service.showInstallationGuide(message, context);
		}
		...
		if (context[CmdCode.Command] === VPNUserCommand.PauseKey) {
			await this.service.pauseKey(message, context);
		}
